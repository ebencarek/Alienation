...
- (void)newBlueAlien
{
    ...
    CGPathAddCurveToPoint(path1, NULL, 0, self.size.height / 1.6, self.size.width, self.size.height / 4, skRand(self.spaceship.size.width / 2, self.size.width - self.spaceship.size.width / 2), 0);
    
    CGMutablePathRef path2 = CGPathCreateMutable();
    CGPathMoveToPoint(path2, NULL, alien.position.x, alien.position.y);
    CGPathAddCurveToPoint(path2, NULL, self.size.width, self.size.height / 1.7, 0, self.size.height / 4.5, skRand(self.spaceship.size.width / 2, self.size.width - self.spaceship.size.width / 2), 0);
    ...
    [alien runAction:[possibleMovements objectAtIndex:(rand() % 2)] withKey:@"move"];
